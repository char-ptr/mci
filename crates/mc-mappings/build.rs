...
fn main() {
    ...
    if let Ok(mut ch) = Command::new("git").arg("clone").arg("--depth").arg("1").arg("--sparse").arg("-b").arg(&mc_ver).arg("https://github.com/FabricMC/yarn").arg(format!("{OUT_DIR}/yarn-maps{mc_ver}")).spawn() {
        ...
        gen.Yarn.run_directory(OUT_DIRP.join(format!("yarn-maps{mc_ver}/mappings")), None).expect("unable to parse yarn mappings");
        let mut tiny = BufReader::new(File::open(OUT_DIRP.join(format!("{mc_ver}.tiny"))).expect("unable to parse tiny mappings"));
