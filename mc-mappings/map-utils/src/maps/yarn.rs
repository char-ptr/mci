use std::{path::PathBuf, fs::File, io::{Read, BufReader, BufRead}, sync::{Arc, RwLock}, borrow::Cow, collections::HashMap};
...
pub fn run_dir<'a>(p:&PathBuf,m:Arc<RwLock<SigMappings<'a>>>, mod_stk : &mut SigMod<'a>, mod_stk_str : &mut Vec<String>) {
    ...
    for entry in dir {
        if let Ok(entry) = entry { // woah, of course using this instead of while would solve all my problems
            let path = entry.path();
            if path.is_dir() {
